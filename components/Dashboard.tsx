...
const Dashboard: React.FC = () => {
  ...
  const cards = [
    { label: 'Total Patients', value: stats.totalPatients, icon: <Users className="text-blue-500" />, bg: 'bg-blue-50' },
    { label: 'Today\'s Appts', value: stats.todayAppointments, icon: <CalendarCheck className="text-purple-500" />, bg: 'bg-purple-50' },
    { label: 'Total Visits', value: stats.totalVisits, icon: <Activity className="text-orange-500" />, bg: 'bg-orange-50' },
    { label: 'Total Revenue', value: `₹${stats.totalRevenue.toLocaleString()}`, icon: <IndianRupee className="text-green-600" />, bg: 'bg-green-50' },
  ];

  // Dummy data for the chart since we don't have historical aggregation in the simple service
  const chartData = [
    { name: 'Mon', visits: 4 },
    { name: 'Tue', visits: 7 },
    { name: 'Wed', visits: 5 },
          ...
          <div className="space-y-4">
            {todayAppointments.length === 0 ? (
              ...
              todayAppointments.map(app => (
                <div key={app.id} className="flex items-start gap-4 p-3 rounded-lg hover:bg-slate-50 transition-colors border-b border-slate-50 last:border-0">
